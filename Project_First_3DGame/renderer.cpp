...
// インスタンス用定数バッファ構造体
struct INSTANCE_CBUFFER
{
	XMMATRIX	World[INSTANCE_MAX];	// ワールド行列
};
...
static void SetInstanceBuffer(void);
...
void SetInstance(INSTANCE* instance, int num)
{
	...
	for (int i = 0; i < num; i++)
	{
		world = XMMatrixTranspose(instance[i].world);
		g_Instance.World[i] = world;
		
	}
	GetDeviceContext()->UpdateSubresource(g_InstanceBuffer, 0, NULL, &g_Instance, 0, 0); 
}
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	//インスタンス
	hBufferDesc.ByteWidth = sizeof(INSTANCE_CBUFFER);
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_InstanceBuffer);
	g_ImmediateContext->VSSetConstantBuffers(8, 1, &g_InstanceBuffer);
	g_ImmediateContext->PSSetConstantBuffers(8, 1, &g_InstanceBuffer);
...
{
	// バックバッファクリア
	float ClearColor[4] = { 0.3f, 0.3f, 0.3f, 1.0f };	// 所謂背景色
